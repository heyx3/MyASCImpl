...
//TODO: Update the GPUGraph repo's RuntimeGraph.cs and GraphUtils.cs!!
...
namespace GPUGraph
{
	...
	public class RuntimeGraph
	{
		...
		/// <summary>
		/// Outputs noise into the given Texture3D.
		/// </summary>
		/// <param name="leaveReadable">
		/// Whether to leave the texture data readable on the CPU after the operation.
		/// </param>
		public void GenerateToTexture(Texture3D outTex, bool leaveReadable = false)
		{
			UpdateAllParams();
			RenderTexture tempTex = RenderTexture.GetTemporary(outTex.width, outTex.height);
			GraphUtils.GenerateToTexture(graphMat, outTex, leaveReadable);
			RenderTexture.ReleaseTemporary(tempTex);
		}
		/// <summary>
		/// Generates to a texture of the given size.
		...
		/// Whether to leave the texture data readable on the CPU after the operation.
		/// </param>
		public Texture3D GenerateToTexture(int width, int height, int depth,
										   TextureFormat fmt = TextureFormat.RGBAFloat,
										   FilterMode filtering = FilterMode.Bilinear,
										   bool mipmaps = true,
										   bool leaveReadable = false)
		{
			UpdateAllParams();
			Texture3D t = new Texture3D(width, height, depth, fmt, mipmaps);
			t.filterMode = filtering;
			GenerateToTexture(t, leaveReadable);
			return t;
		}
