...
namespace GPUGraph
{
	...
	public static class GraphUtils
	{
        ...
        public static void GenerateToTexture(Material noiseMat, Texture3D copyTo,
										     bool leaveTextureReadable)
        {
            ...
            Color32[] finalPixels = new Color32[width * height * copyTo.depth];
            SetUpColorTex(width, height);
			float invDepth = 1.0f / copyTo.depth;
            for (int depthI = 0; depthI < copyTo.depth; ++depthI)
            {
                //Get the UV.z coordinate.
                float uvZ = (float)depthI * invDepth;
